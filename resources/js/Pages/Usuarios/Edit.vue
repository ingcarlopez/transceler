  ...
  <div>
    ...
    <div class="flex justify-start mb-8 max-w-3xl">
      <h1 class="text-3xl font-bold">
        <Link class="text-rose-400 hover:text-rose-600" href="/users">Users</Link>
    ...
    <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
      <form @submit.prevent="update">
        ...
        <div class="flex items-center px-8 py-4 bg-gray-50 border-t border-gray-100">
          ...
          <loading-button :loading="form.processing" class="btn-rose ml-auto" type="submit">Update User</loading-button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    destroy() {
      if (confirm('Está seguri que desea eliminar este user?')) {
        this.$inertia.delete(`/users/${this.user.id}`)
      }
    },
    restore() {
      if (confirm('Está seguri que desea restaurar este user?')) {
